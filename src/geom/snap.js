...
export function snapPoint(p, { points = [], tol = 150, anchor = null, ortho = false, snap = true } = {}) {
  ...
  const lockX = ortho && anchor && point[1] === anchor[1] ? false : true;
  const lockY = ortho && anchor && point[0] === anchor[0] ? false : true;
  const vx = lockX ? points.find(q => Math.abs(q[0] - point[0]) <= tol) : null;
  const hy = lockY ? points.find(q => Math.abs(q[1] - point[1]) <= tol) : null;
