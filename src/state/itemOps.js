...
import { uid, normalizeItem, activeFloor } from './schema.js';
import { placeOnWall, isEmbed } from '../geom/items.js';
import { itemsOf, updateItems } from './floorOps.js';
...
export function mirrorItems(store, ids, axis, opts = {}) {
  const items = itemsOf(store.get(), ids);
...
export function pasteItems(store, snapshots, { delta = [200, 200] } = {}) {
  const copies = (snapshots ?? []).map(i => normalizeItem({ ...structuredClone(i), id: uid('i'), pos: [i.pos[0] + delta[0], i.pos[1] + delta[1]] }));
