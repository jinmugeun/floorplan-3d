...
export function pickAt(store, ui, p, { scale = 1 } = {}) {
  ...
  const hd = hitDuct(ducts, p, px(PICK_TOL_PX.duct));
  if (hd) return { type: 'duct', ductId: hd.ductId, vertex: hd.vertex ?? null, segment: hd.segment ?? null, t: hd.t ?? null, handle: false };
  // 위에 그린 아이템이 먼저 잡힌다(drawOrder 역순 = 천장 부착 먼저). 잠긴 아이템은 클릭으로 잡히지
  // 않는다(레이어 패널에서만 고른다 — itemDrag.pick과 같은 규칙).
  const order = drawOrder(f.items);
  for (let i = order.length - 1; i >= 0; i--) {
    const it = order[i];
    if (it.locked || !itemVisible(it, flags)) continue;
    if (pointInItem(p, it, px(PICK_TOL_PX.item))) return { type: 'item', item: it };
  }
