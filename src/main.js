...
const shell = createShell(document.getElementById('app'), { store, ui, onGizmoMode: m => view3d.setGizmoMode(m), onMinimapResize: () => minimap?.requestRender(), onOpenKeymap: () => openSettingsDialog({ store, tab: 'keys' }), onExitFp: () => setMode('iso'), onToolChange: () => view.requestRender(), onPanelShow: name => { if (name === 'layers') layersPanel?.reveal(); }, onLabelDensity: () => view3d.refreshLabels(), onFirstRoom: () => setTool('room') });
