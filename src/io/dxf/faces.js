...
export function largestCluster(segs, link = ROI_LINK) {
  ...
  for (const s of segs) for (const p of [s.a, s.b]) { const k = cell(p); if (!par.has(k)) par.set(k, k); }
  for (const s of segs) uni(cell(s.a), cell(s.b));                       // ①
...
export function buildFaces(segs, P = DXF_PARAMS) {
  ...
  for (const [key, list] of bins) {
    const u = [list.reduce((a, x) => a + x.u[0], 0) / list.length, list.reduce((a, x) => a + x.u[1], 0) / list.length];
