...
const DRAG_END_EVENTS = ['pointerup', 'pointercancel', 'lostpointercapture'];
...
export function createShell(root, { store, ui, onGizmoMode = () => {}, onMinimapResize = () => {}, onOpenKeymap = () => {} }) {
  ...
  // 캔버스 위에서 포인터를 누르고 있는 동안을 "드래그 중"으로 본다(renderBanner 아래 주석 참고).
  let dragging = false;
  ...
  function bannerHtml(s) {
    ...
    if (currentTool?.hint) return `<button type="button" class="hint" data-action="hintCancel">${esc(currentTool.hint)}</button>`;
    return '';
  }
  let bannerLast = null;
  function renderBanner(s = ui.get()) {
    ...
    if (dragging && (!html) !== els.banner.hidden) { pendingBanner = true; return; }
    if (html === bannerLast) return;   // 같은 문구를 다시 쓰면 배너 안 버튼(취소·도면 전체 보기)의 포커스가 날아간다
    bannerLast = html;
    els.banner.hidden = !html;
    els.banner.innerHTML = html;
    if (html.includes('btnExitSolo')) q('#btnExitSolo').onclick = () => ui.set({ soloRoom: null });
  }
  ...
  q('#canvasStack').addEventListener('pointerdown', () => { dragging = true; });
  // pointerup은 포인터 캡처 덕에 캔버스로 되돌아오지만, 창 밖에서 손을 떼는 경우까지 덮으려면
  // window에서 받아야 한다(놓친 pointerup 하나가 배너를 영구히 얼린다 — capture 단계로 확실히 받는다).
  const onDragEnd = () => { if (!dragging) return; dragging = false; if (pendingBanner) { pendingBanner = false; renderBanner(); } };
  for (const t of DRAG_END_EVENTS) globalThis.addEventListener?.(t, onDragEnd, true);
  ...
  return { els, setOptionBar, showPanel, setOrtho, toast, popover: pop, refreshPopover, refreshBanner: () => renderBanner(), destroy() { bottom?.destroy(); resizeWatch.destroy(); splitters.forEach(s => s.destroy()); for (const t of DRAG_END_EVENTS) globalThis.removeEventListener?.(t, onDragEnd, true); } };
}
