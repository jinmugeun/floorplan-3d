...
// [벽 후보만] = 자동 판정이 벽이라고 본 켜진 레이어 — 그 규칙의 주인은 classify.js의
// defaultChecked 하나다(리뷰 I-2: 같은 필터를 여기에 베껴 두면 한쪽만 고쳐질 자리가 생긴다).
// [전체] = 꺼지지 않은 레이어 전부. 꺼진 레이어는 [전체]에도 들어가지 않는다 — 실측에서 최상위
// 엔티티의 67 %가 거기에 있고, 그것을 켜면 모델스페이스가 844 m × 2,229 m로 벌어진다.
export const wallOnly = rows => defaultChecked(rows);
