...
export function confirmDialog({ title = '확인', message = '', ok = '확인', cancel = '취소', danger = false } = {}) {
  return new Promise(resolve => {
    ...
    // 캡처 단계에서 듣고 멈춘다: Esc·Enter가 전역 단축키(도구 전환·저장)까지 내려가지 않게.
    function onKey(ev) {
      if (ev.key === 'Escape') { ev.preventDefault(); ev.stopPropagation(); close(false); return; }
      if (ev.key === 'Enter') { ev.preventDefault(); ev.stopPropagation(); close(true); return; }
    ...
    root.querySelector('[name="ok"]').focus();
  });
}
