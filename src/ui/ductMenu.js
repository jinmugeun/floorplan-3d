...
export function ductMenuItems({ store, ui, sel, at = null }) {
  ...
  return [
    ...
    { label: '점 삽입', ...why(d.locked ? WHY_LOCKED_DUCT : seg === null ? WHY_NO_SEGMENT : null), onSelect: () => { insertDuctPoint(store, d.id, seg, insertAt); clear(); } },
    { label: '점 삭제', ...why(d.locked ? WHY_LOCKED_DUCT : vtx === null ? WHY_NO_VERTEX : d.points.length <= 2 ? WHY_MIN_POINTS : null), onSelect: () => { deleteDuctPoint(store, d.id, vtx); clear(); } },
    { label: '댐퍼 추가', ...why(d.locked ? WHY_LOCKED_DUCT : seg === null ? WHY_NO_SEGMENT : null), onSelect: () => addDamper(store, d.id, { segment: seg, t: 0.5, type: 'VD' }) },
    'sep',
    { label: d.kind === 'supply' ? '배기로 전환' : '급기로 전환', ...why(d.locked ? WHY_LOCKED_DUCT : null), onSelect: () => updateDuct(store, d.id, { kind: d.kind === 'supply' ? 'exhaust' : 'supply' }) },
    { label: '설비 연결 해제', ...why(conn ? null : WHY_NO_CONNECTION), onSelect: () => disconnectDuct(store, d.id, vtx) },
    'sep',
