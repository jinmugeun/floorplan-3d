...
// 그리기 체인의 배너 한 줄(§17.8(4) · 감사 §52). [Enter]는 한 번이다 — 같은 키가 "확정"과 "완료"를
// 함께 뜻하지 않게, 확정할 것이 없을 때만 그리기가 끝난다. "취소"가 아니라 "그리기 끝"인 이유:
// 이미 놓인 구간은 각각 되돌리기 단계로 남아 있고 [Esc]는 그것을 지우지 않는다.
export const DRAW_CHAIN_HINT = `다음 점을 클릭 · ${TYPED_DIM_HINT} 확정 · [Esc] 그리기 끝`;

// 마감재를 고르지 않은 면에서 오프셋·각도 칸을 고치면 바를 자리가 없다(§17.4(1) · 리뷰 I-1·I-2).
// 예전에는 레거시 문자열에서 만든 파생값을 문서에 굳혀, 고른 적 없는 마감재가 조용히 발렸다.
export const MATERIAL_PICK_FIRST = '먼저 마감재를 고르세요 — 지정된 재질이 없는 면에는 오프셋·각도를 바를 자리가 없습니다';

// 길이는 넣었는데 방향이 아직 없는 프레임(커서가 마지막 점 위)의 배너(재리뷰 NEW-1 · §17.8).
...
// 가져오기 자체가 실패했을 때의 한 줄(리뷰 F-3). DXF_ERRORS 다섯은 §18.6의 표가 동결한 파싱·추출
// 오류라 여기에 섞지 않는다 — 이 문장은 호출자(store.swap·자동 저장)가 거절했을 때만 나오고,
// 가장 흔한 원인인 트레이스 배경(최대 1.2 MB)을 사용자가 이 대화상자에서 바로 끌 수 있게 말한다.
export const DXF_IMPORT_FAILED = '가져오기를 마치지 못했습니다. 원 도면을 배경으로 남기지 않고 다시 시도해 주세요';
